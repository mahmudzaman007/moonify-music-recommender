...
def recommend_dynamic(track_name, n_recommendations=5):
    ...
    matches = scaled_features_df[scaled_features_df['track_name'] == track_name]
    if matches.empty:
    ...
    # Get sorted indices (most similar first, exclude itself)
    sorted_indices = similarity_scores.argsort()[::-1]
    top_indices = [i for i in sorted_indices if i != track_index][:n_recommendations]
